...
def calc_metrics(actual, predicted):
    ...
    return {
        ...
        'MASE':mase,'R2':1-np.sum(se)/np.sum((a-a.mean())**2) if var_a>0 else None,
        'TheilU':theil_u}
...
def show_param_table(result):
    ...
    for idx_num, name in enumerate(params.index):
        rows.append({
            'Parameter':name,'Estimate':f"{params[name]:.6f}",
...
# ============================================================
    ...
    nt=int(len(ym)*split_pct/100)
    y_tr=ym.iloc[:nt]; y_te=ym.iloc[nt:]
    ...
    if st.button("Forecast",type="primary"):
        with st.spinner("Forecasting..."):
            try:
                ...
                resid_std=model.resid.std(); z=stats.norm.ppf(1-alpha/2)
