...
import warnings, io, itertools, subprocess, sys
...
try:
    ...
    st.sidebar.warning("⚠️ Package `arch` belum terinstall.")
    if st.sidebar.button("Install `arch` sekarang"):
        with st.spinner("Installing arch..."):
            subprocess.check_call([sys.executable, "-m", "pip", "install", "arch", "-q"])
        st.success("✅ `arch` berhasil diinstall! Silakan refresh halaman.")
        st.rerun()
...
def load_demo_stock():
    ...
    return pd.DataFrame({'Date':pd.date_range('2023-01-02',periods=n,freq='B'),
                         'Price':np.round(price,2),'Volume':np.random.randint(1e6,5e6,n)})

@st.cache_data
def load_demo_sales():
    np.random.seed(99); n=120; t=np.arange(n)
    trend=500+3*t; seas=80*np.sin(2*np.pi*t/12)+40*np.cos(2*np.pi*t/6)
    promo=np.zeros(n); promo[np.random.choice(n,20,replace=False)]=np.random.uniform(50,150,20)
    y=trend+seas+promo+np.random.normal(0,20,n)
...
def calc_metrics(actual, predicted):
    """Calculate comprehensive metrics, returns dict of floats"""
    ...
    if len(a)==0: return {k:None for k in ['ME','MAE','MSE','RMSE','MAPE','MdAPE','MASE','R2','AdjR2','TheilU']}
    e=a-p; ae=np.abs(e); se=e**2
    nz=a!=0; ape=np.abs(e[nz]/a[nz])*100 if nz.sum()>0 else np.array([])
    n=len(a); var_a=np.var(a)
    # Naive forecast (random walk) for MASE
    ...
    theil_u=num/den if den>0 else None
    return {
        'ME':np.mean(e),'MAE':np.mean(ae),'MSE':np.mean(se),'RMSE':np.sqrt(np.mean(se)),
        'MAPE':np.mean(ape) if len(ape)>0 else None,
        'MdAPE':np.median(ape) if len(ape)>0 else None,
        'MASE':mase,
        'R2':1-np.sum(se)/np.sum((a-a.mean())**2) if var_a>0 else None,
        'TheilU':theil_u
    }

def display_three_metrics(y_train, fitted_train, y_test, forecast_test):
    """Display In-Sample, Out-Sample, Overall metrics in 3 columns"""
    in_m  = calc_metrics(y_train, fitted_train)
    out_m = calc_metrics(y_test, forecast_test)
    # Overall = combine both
    all_actual = np.concatenate([np.array(y_train,dtype=float), np.array(y_test,dtype=float)])
    all_pred   = np.concatenate([np.array(fitted_train,dtype=float), np.array(forecast_test,dtype=float)])
    ov_m = calc_metrics(all_actual, all_pred)
    ...
    for key in ['ME','MAE','MSE','RMSE','MAPE','MdAPE','MASE','R2','TheilU']:
        label = key
        if key == 'MAPE': label = 'MAPE (%)'
        if key == 'MdAPE': label = 'MdAPE (%)'
        rows.append({
            'Metric': label,
            'In-Sample (Train)': fmt(in_m.get(key)),
            'Out-Sample (Test)': fmt(out_m.get(key)),
            'Overall': fmt(ov_m.get(key))
        })
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

def write_model_notation(model_type, order, seasonal_order=None, exog_names=None, transform=None):
    """Auto-generate mathematical model notation"""
    p,d,q = order
    parts = []

    # Transform label
    if transform and transform != 'None':
        tf = {'Log':'\\ln','Sqrt':'\\sqrt{}','Box-Cox':'\\text{BoxCox}'}.get(transform, '')
    else:
        tf = ''
    ...
    diff_str = ""
    if d == 1: diff_str = "(1-B)"
    elif d == 2: diff_str = "(1-B)^2"
    elif d > 0: diff_str = f"(1-B)^{d}"
    ...
    if seasonal_order:
        ...
        elif D > 0: seas_diff_str = f"(1-B^{{{s}}})^{D}"

    # AR polynomial
    ar_str = ""
    if p > 0:
        ar_terms = " - ".join([f"\\phi_{{{i}}}B^{{{i}}}" if i>1 else f"\\phi_1 B" for i in range(1,p+1)])
        ar_str = f"(1 - {ar_terms})"

    # MA polynomial
    ma_str = ""
    if q > 0:
        ma_terms = " + ".join([f"\\theta_{{{i}}}B^{{{i}}}" if i>1 else f"\\theta_1 B" for i in range(1,q+1)])
        ma_str = f"(1 + {ma_terms})"

    # Seasonal AR
    sar_str = ""
    if seasonal_order and seasonal_order[0] > 0:
        P_val = seasonal_order[0]; s = seasonal_order[3]
        sar_terms = " - ".join([f"\\Phi_{{{i}}}B^{{{i*s}}}" for i in range(1,P_val+1)])
        sar_str = f"(1 - {sar_terms})"

    # Seasonal MA
    sma_str = ""
    if seasonal_order and seasonal_order[2] > 0:
        Q_val = seasonal_order[2]; s = seasonal_order[3]
        sma_terms = " + ".join([f"\\Theta_{{{i}}}B^{{{i*s}}}" for i in range(1,Q_val+1)])
...
def show_model_notation(label, equation):
    """Display model notation in Streamlit"""
    st.markdown(f"### Model: **{label}**")
    st.latex(equation)

def plot_acf_pacf(series, lags=40, title=""):
    sc = series.dropna()
    ml = min(lags, len(sc)//2-2)
    if ml < 1: ml = 1
    a_v = acf(sc, nlags=ml, fft=True)
    p_v = pacf(sc, nlags=ml)
    ci = 1.96/np.sqrt(len(sc))
    ...
    for i, vals in enumerate([a_v, p_v]):
        ...
        for j in range(len(vals)):
            ...
            fig.add_trace(go.Bar(x=[j],y=[vals[j]],marker_color=c,showlegend=False,width=0.3),row=1,col=col)
...
def show_param_table(result):
    """Show fitted parameter table from statsmodels result"""
    params = result.params
    se = result.bse
    zvals = result.zvalues
    ...
    for name in params.index:
        rows.append({
            'Parameter': name,
            'Estimate': f"{params[name]:.6f}",
            'Std Error': f"{se[name]:.6f}" if name in se.index else '-',
            'z-value': f"{zvals[name]:.4f}" if name in zvals.index else '-',
            'p-value': f"{pvals[name]:.6f}" if name in pvals.index else '-',
            'CI Lower': f"{ci.iloc[list(params.index).index(name), 0]:.6f}" if name in params.index else '-',
            'CI Upper': f"{ci.iloc[list(params.index).index(name), 1]:.6f}" if name in params.index else '-',
            'Signif': '***' if pvals.get(name,1)<0.001 else ('**' if pvals.get(name,1)<0.01 else ('*' if pvals.get(name,1)<0.05 else ''))
        })
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
...
if module=='eda':
    ...
    fig=go.Figure(); fig.add_trace(go.Scatter(x=y_full.index,y=y_full.values,mode='lines',line=dict(color='steelblue',width=1.5)))
    fig.update_layout(title=f"Time Series: {target_col}",height=400,template='plotly_white'); st.plotly_chart(fig,use_container_width=True)
    ...
    if hasattr(y_full.index,'month'):
        ...
        fig3.update_layout(title="Average by Month",height=320,template='plotly_white'); st.plotly_chart(fig3,use_container_width=True)
...
# ============================================================
    ...
    if method=='Classical':
        ...
        if model_type=='multiplicative':
            st.info("STL hanya additive. Data di-log untuk approximate multiplicative.")
    ...
    vr=resid.var(); ts=pd.Series(decomp.trend).dropna(); ss=pd.Series(decomp.seasonal).dropna()
...
# ============================================================
    ...
    y_test,bc_lam=apply_transformation(y_full,transform)
    if len(y_test)<10: st.error("Data terlalu sedikit setelah transformasi."); st.stop()
    ...
    adf_r=adf_test(y_test); st.dataframe(pd.DataFrame([adf_r]),use_container_width=True,hide_index=True)
    if adf_r['Stationary']=='Yes': st.success("ADF: Stationary")
    else: st.warning("ADF: Non-Stationary")
    st.markdown("### KPSS (H₀: Stationary)")
    kpss_r=kpss_test(y_test); st.dataframe(pd.DataFrame([kpss_r]),use_container_width=True,hide_index=True)
    ...
    else: st.error("**NON-STATIONARY** — perlu differencing")
    if not adf_ok:
        st.markdown("### Auto-Differencing Suggestion")
        d=0; temp=y_full.copy()
        for dd in range(1,4):
            temp=temp.diff().dropna()
            if len(temp)<10: break
            if adfuller(temp,autolag='AIC')[1]<0.05: d=dd; break
        if d>0: st.info(f"Suggested **d = {d}**")
        else: st.warning("Masih non-stationary setelah 3x differencing.")
...
# ============================================================
    ...
    if len(y_plot)<10: st.error("Data terlalu sedikit."); st.stop()
    ml=st.slider("Max lags:",5,min(80,len(y_plot)//2-2),min(40,len(y_plot)//3))
    st.plotly_chart(plot_acf_pacf(y_plot,lags=ml,title=f"({transform})"),use_container_width=True)
    fig2=go.Figure(); fig2.add_trace(go.Scatter(x=y_plot.index,y=y_plot.values,mode='lines',line=dict(width=1)))
    fig2.update_layout(title=f"Series ({transform})",height=300,template='plotly_white'); st.plotly_chart(fig2,use_container_width=True)
...
# ============================================================
    ...
    if len(y_model)<10: st.error("Data terlalu sedikit."); st.stop()
    ...
    if st.button("Fit ARIMA",type="primary"):
        with st.spinner("Fitting..."):
            try:
                ...
                fc_ci=fc_obj.conf_int(alpha=0.05); ci_cols=fc_ci.columns.tolist()
                ...
                fig2.add_trace(go.Scatter(x=ci_l.index.tolist()+ci_u.index.tolist()[::-1],
                ...
                # 3-WAY METRICS
                st.markdown("### Model Evaluation (In-Sample / Out-Sample / Overall)")
                display_three_metrics(y_train.values, fitted.values, y_test.values, fc_mean.values)
                ...
                try:
                    ...
                    st.markdown(f"**ARCH Effect:** LM={ar_r[0]:.4f}, p={ar_r[1]:.6f} → {'No ARCH ✓' if ar_r[1]>0.05 else 'ARCH detected → Try GARCH! ✗'}")
                except: pass
    ...
    if st.button("Fit SARIMA",type="primary"):
        with st.spinner("Fitting..."):
            try:
                ...
                fitted=result.fittedvalues
                fc_obj=result.get_forecast(steps=len(y_test))
                fc_mean=fc_obj.predicted_mean; fc_ci=fc_obj.conf_int(alpha=0.05); ci_cols=fc_ci.columns.tolist()
...
# ============================================================
    ...
    y_model=y_full.loc[ci_idx]; X=X.loc[ci_idx]
    ...
    if st.button("Fit ARIMAX",type="primary"):
        with st.spinner("Fitting..."):
            try:
                ...
                fc_mean=fc.predicted_mean; fc_ci=fc.conf_int(alpha=0.05); ci_cols=fc_ci.columns.tolist()
...
# ============================================================
    ...
    if not HAS_ARCH:
        st.error("Package `arch` belum terinstall.")
        st.markdown("### Install Manual")
        st.code("pip install arch", language="bash")
        st.markdown("Atau klik tombol **Install** di sidebar, lalu refresh.")
        st.stop()

    use_returns=st.checkbox("Gunakan log-returns?",True)
    if use_returns:
        yp=y_full[y_full>0]; y_model=(np.log(yp).diff().dropna()*100)
        st.info("Using log-returns (×100)")
    else:
        y_model=y_full.copy()
    if len(y_model)<30: st.error("Data terlalu sedikit."); st.stop()

    fig=go.Figure(); fig.add_trace(go.Scatter(x=y_model.index,y=y_model.values,mode='lines',line=dict(width=0.8)))
    ...
    if st.button("Fit GARCH",type="primary"):
        with st.spinner("Fitting..."):
            try:
                ...
                am=arch_model(y_train,mean='ARX',lags=ar,vol=vol_map[vol_model],p=gp,q=gq,o=o_val,dist=dist)
                res=am.fit(disp='off')

                # Auto notation for GARCH
                st.markdown(f"### Model: **AR({ar})-{vol_model}({gp},{gq})** [{dist} distribution]")
                mean_eq = f"r_t = \\mu"
                if ar > 0:
                    ar_terms = " + ".join([f"\\phi_{{{i}}} r_{{t-{i}}}" for i in range(1,ar+1)])
                    mean_eq += f" + {ar_terms}"
                ...
                # Parameter table
                st.markdown("### Parameter Estimates")
                garch_params = res.params
                garch_se = res.std_err
                garch_pv = res.pvalues
                rows=[]
                for nm in garch_params.index:
                    rows.append({
                        'Parameter':nm,
                        'Estimate':f"{garch_params[nm]:.6f}",
                        'Std Error':f"{garch_se[nm]:.6f}" if nm in garch_se.index else '-',
                        'p-value':f"{garch_pv[nm]:.6f}" if nm in garch_pv.index else '-',
                        'Signif':'***' if garch_pv.get(nm,1)<0.001 else ('**' if garch_pv.get(nm,1)<0.01 else ('*' if garch_pv.get(nm,1)<0.05 else ''))
                    })
                st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
                ...
                fc_mean=fc.mean.iloc[-1].values; fc_var=fc.variance.iloc[-1].values; fc_vol=np.sqrt(fc_var)
                ...
                fig2.add_trace(go.Scatter(x=y_test.index[:h].tolist()+y_test.index[:h].tolist()[::-1],
                ...
                # 3-way metrics for GARCH
                # In-sample: fitted = residuals subtracted from actual
                in_fitted = y_train.values - res.resid.values
                st.markdown("### Evaluation (In-Sample / Out-Sample / Overall)")
                display_three_metrics(y_train.values, in_fitted, y_test.values[:h], fc_mean[:h])
...
# ============================================================
    ...
    if transform=='Log' and (y_full>0).all(): y_model=np.log(y_full)
    else: y_model=y_full.copy()
            ...
            for idx,combo in enumerate(combos):
                ...
                try:
                    if seasonal:
                        ...
                        m=SARIMAX(y_train,order=(pp,dd,qq),seasonal_order=(PP,DD,QQ,s_period),
                                  enforce_stationarity=False,enforce_invertibility=False)
                    else:
                        pp,dd,qq=combo
                        if pp==0 and qq==0: continue
                        m=ARIMA(y_train,order=(pp,dd,qq))
                    res=m.fit(disp=False,maxiter=200)
                    fitted_vals=res.fittedvalues
                    fc=res.get_forecast(steps=len(y_test)); fc_mean=fc.predicted_mean
                    in_m=calc_metrics(y_train.values,fitted_vals.values)
                    out_m=calc_metrics(y_test.values,fc_mean.values)
                    row={'p':pp,'d':dd,'q':qq,
                        'AIC':round(res.aic,2),'BIC':round(res.bic,2),
                        'RMSE_train':round(in_m.get('RMSE',999),4),
                        'RMSE_test':round(out_m.get('RMSE',999),4),
                        'MAE_test':round(out_m.get('MAE',999),4),
                        'MAPE_test':round(out_m['MAPE'],2) if out_m.get('MAPE') is not None else None}
...
# ============================================================
    ...
    models_sel=st.multiselect("Models:",[
    ...
    if st.button("Compare",type="primary") and models_sel:
        all_results=[]; forecasts={}
        for mname in models_sel:
            try:
                fc=None; aic=None; bic=None; fitted_v=None
                if mname.startswith('ARIMA('):
                    order=tuple(int(x) for x in mname.replace('ARIMA','').strip('()').split(','))
                    m=ARIMA(y_train,order=order).fit()
                    fc=m.get_forecast(len(y_test)).predicted_mean; fitted_v=m.fittedvalues.values; aic=m.aic; bic=m.bic
                    ...
                    try: m=ExponentialSmoothing(y_train,trend='add',seasonal=st_type,seasonal_periods=12).fit()
                    except: m=ExponentialSmoothing(y_train,trend='add',seasonal='add',seasonal_periods=12).fit()
                    fc=m.forecast(len(y_test)); fitted_v=m.fittedvalues.values; aic=m.aic; bic=m.bic
                elif mname=='Naive':
                    fc=pd.Series(np.full(len(y_test),y_train.iloc[-1]),index=y_test.index)
                    fitted_v=np.full(len(y_train),y_train.iloc[-1])
                elif mname=='Drift':
                    nt=len(y_train); drift=(y_train.iloc[-1]-y_train.iloc[0])/(nt-1) if nt>1 else 0
                    fc=pd.Series([y_train.iloc[-1]+drift*(i+1) for i in range(len(y_test))],index=y_test.index)
                    fitted_v=np.array([y_train.iloc[0]+drift*i for i in range(nt)])

                if fc is not None:
                    in_m=calc_metrics(y_train.values, fitted_v) if fitted_v is not None else {k:None for k in ['RMSE','MAE','MAPE']}
                    out_m=calc_metrics(y_test.values,fc.values)
                    all_results.append({
                        'Model':mname,
                        'AIC':f"{aic:.2f}" if aic else '-', 'BIC':f"{bic:.2f}" if bic else '-',
                        'RMSE (Train)':f"{in_m.get('RMSE',0):.4f}" if in_m.get('RMSE') is not None else '-',
                        'RMSE (Test)':f"{out_m.get('RMSE',0):.4f}",
                        'MAE (Test)':f"{out_m.get('MAE',0):.4f}",
                        'MAPE (Test)':f"{out_m['MAPE']:.2f}%" if out_m.get('MAPE') is not None else '-',
                        'R2 (Test)':f"{out_m['R2']:.4f}" if out_m.get('R2') is not None else '-'
                    })
                    forecasts[mname]=fc
            except Exception as e:
                all_results.append({'Model':mname,'AIC':'-','BIC':'-',
                    'RMSE (Train)':'-','RMSE (Test)':str(e)[:30],'MAE (Test)':'-','MAPE (Test)':'-','R2 (Test)':'-'})

        if all_results: st.dataframe(pd.DataFrame(all_results),use_container_width=True,hide_index=True)
        if forecasts:
            fig=go.Figure()
    ...
    if st.button("Forecast",type="primary"):
        with st.spinner("Fitting & forecasting..."):
            try:
                ...
                fc_mean=fc_obj.predicted_mean
                fc_ci=fc_obj.conf_int(alpha=alpha); ci_cols=fc_ci.columns.tolist()
                ci_l=fc_ci[ci_cols[0]]; ci_u=fc_ci[ci_cols[1]]

                resid_std=model.resid.std()
                z=stats.norm.ppf(1-alpha/2)
                ci_hw=(ci_u.values-fc_mean.values)
                pi_hw=np.sqrt(ci_hw**2+resid_std**2)
                pi_l_vals=fc_mean.values-z*np.sqrt(ci_hw**2/z**2+resid_std**2)
                pi_u_vals=fc_mean.values+z*np.sqrt(ci_hw**2/z**2+resid_std**2)

                if transform=='Log':
                    fcp=np.exp(fc_mean); cl=np.exp(ci_l); cu=np.exp(ci_u)
                    pl=np.exp(pi_l_vals); pu=np.exp(pi_u_vals); yp=np.exp(y_model)
                else:
                    fcp=fc_mean; cl=ci_l; cu=ci_u; pl=pi_l_vals; pu=pi_u_vals; yp=y_model

                fig=go.Figure()
                fig.add_trace(go.Scatter(x=yp.index,y=yp.values,mode='lines',name='Historical',line=dict(color='steelblue',width=1.5)))
                fig.add_trace(go.Scatter(x=fcp.index,y=fcp.values,mode='lines',name='Forecast',line=dict(color='crimson',width=2)))
                pl_v=pl if isinstance(pl,np.ndarray) else pl.values
                pu_v=pu if isinstance(pu,np.ndarray) else pu.values
                fig.add_trace(go.Scatter(x=fcp.index.tolist()+fcp.index.tolist()[::-1],
                    y=list(pl_v)+list(pu_v)[::-1],fill='toself',fillcolor='rgba(255,165,0,0.08)',
                    line=dict(color='rgba(0,0,0,0)'),name=f'{ci_label} PI'))
                cl_v=cl.values if hasattr(cl,'values') else cl; cu_v=cu.values if hasattr(cu,'values') else cu
                fig.add_trace(go.Scatter(x=fcp.index.tolist()+fcp.index.tolist()[::-1],
                    y=list(cl_v)+list(cu_v)[::-1],fill='toself',fillcolor='rgba(255,0,0,0.15)',
                    line=dict(color='rgba(0,0,0,0)'),name=f'{ci_label} CI'))
                ...
                fc_df=pd.DataFrame({
                    'Date':fc_mean.index,
                    'Forecast':np.round(fcp_v,4),
                    'CI Lower':np.round(cl_v,4),'CI Upper':np.round(cu_v,4),
                    'PI Lower':np.round(pl_v,4),'PI Upper':np.round(pu_v,4)})
