...
with tab2:
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        # Jarque-Bera
        jb_stat, jb_p, skew_val, kurt_val = jarque_bera(residuals)
        with col2:
...
with tab3:
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        y_ols = df[dep].values.reshape(-1, 1)
        ...
        equation = f"**{dep}** = {ols_model.params[0]:.4f}"
        ...
        try:
            latex_str = f"{dep} = {ols_model.params[0]:.4f}"
            for ii, vv in enumerate(indeps):
                cv = ols_model.params[ii+1]
                sgn = "+" if cv >= 0 else "-"
                latex_str += f" {sgn} {abs(cv):.4f} \\cdot \\text{{{vv}}}"
            st.latex(latex_str)
        except Exception:
            pass
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        if st.session_state.gwr_results is not None:
            ...
            **Form umum GWR:**
            """)
            try:
                gwr_latex = "y_i = \\beta_0(u_i, v_i)"
                for jj in range(len(indeps)):
                    gwr_latex += f" + \\beta_{{{jj+1}}}(u_i, v_i) \\cdot X_{{{jj+1}i}}"
                gwr_latex += " + \\varepsilon_i"
                st.latex(gwr_latex)
            except Exception:
                st.markdown("**y_i = β₀(u_i,v_i) + Σ βₖ(u_i,v_i)·Xₖᵢ + εᵢ**")

            st.markdown("**Dimana koefisien rata-rata:**")
            try:
                eq_parts = [f"{params[:, 0].mean():.4f}"]
                for jj, vn in enumerate(indeps):
                    coef_mean = params[:, jj+1].mean()
                    sgn = "+" if coef_mean >= 0 else "-"
                    eq_parts.append(f"{sgn} {abs(coef_mean):.4f} x {vn}")
                st.markdown(f"**y = {' '.join(eq_parts)}**")
            except Exception:
                pass
            ...
            with st.expander("📄 Full GWR Summary"):
                try:
                    ...
                    _sys.stdout = _old
                    _txt = _buf.getvalue()
                    st.text(_txt if _txt.strip() else "Summary tidak tersedia.")
...
# (folium/geopandas imports moved to top)
