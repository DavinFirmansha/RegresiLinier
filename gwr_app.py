...
with tab3:
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        st.latex(
            f"{dep} = {ols_model.params[0]:.4f}" +
            "".join([
                f" {'+ ' if ols_model.params[i+1] >= 0 else '- '}{abs(ols_model.params[i+1]):.4f} \\cdot \\text{{{v}}}"
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        if st.session_state.gwr_results is not None:
            ...
            **Form umum GWR:**
            """)
            st.latex(
                f"y_i = \\beta_0(u_i, v_i) + "
                + " + ".join([f"\\beta_{{{j+1}}}(u_i, v_i) \\cdot X_{{{j+1}i}}" for j in range(len(indeps))])
                + " + \\varepsilon_i"
            )

            st.markdown("**Dimana koefisien rata-rata:**")
            eq_mean = f"$\\hat{{y}}_i = {params[:, 0].mean():.4f}$"
            for j, vn in enumerate(indeps):
                coef_mean = params[:, j+1].mean()
                sign = "+" if coef_mean >= 0 else "-"
                eq_mean += f" ${sign} {abs(coef_mean):.4f} \\times {vn}$"
            st.markdown(eq_mean)
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        def create_colormap(series, cmap_name='RdYlBu', n_bins=7):
            ...
            return cm.LinearColormap(
                colors=plt.get_cmap(cmap_name)(np.linspace(0, 1, n_bins)),
                vmin=vmin, vmax=vmax
            )
