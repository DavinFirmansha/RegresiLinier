...
# ============================================================
# HELPER: Compute Local R2 for MGWR (not natively supported)
# ============================================================
def compute_local_r2(y, predy):
    """Compute pseudo local R2 = 1 - (residual^2 / SStotal_per_obs)."""
    y_flat = y.flatten()
    pred_flat = predy.flatten()
    ss_tot = np.sum((y_flat - y_flat.mean()) ** 2)
    if ss_tot == 0:
        return np.zeros_like(y_flat)
    residuals = y_flat - pred_flat
    # Global R2 applied uniformly (MGWR doesn't support per-location R2)
    global_r2 = 1 - np.sum(residuals ** 2) / ss_tot
    return np.full_like(y_flat, global_r2, dtype=float)

def safe_get_localR2(model_results, y=None):
    """Safely get localR2, falling back to manual computation for MGWR."""
    try:
        return model_results.localR2.flatten()
    except (NotImplementedError, AttributeError):
        if y is not None and hasattr(model_results, 'predy'):
            return compute_local_r2(y, model_results.predy)
        return None
...
with tab5:
    ...
    if st.session_state.dep_var and st.session_state.indep_vars:
        ...
        if st.session_state.mgwr_results is not None:
            ...
            _mgwr_lr2 = safe_get_localR2(mgwr_results, st.session_state.y)
            if _mgwr_lr2 is not None:
                export_mgwr['MGWR_LocalR2'] = _mgwr_lr2
...
with tab7:
    ...
    if not FOLIUM_AVAILABLE:
        ...
        if map_model == "OLS":
            ...
            localR2_m = safe_get_localR2(mgwr, st.session_state.y)

            var_names = ['Intercept'] + indeps
            for i, vn in enumerate(var_names):
                map_df[f'MGWR_Beta_{vn}'] = params_m[:, i]
                map_df[f'MGWR_t_{vn}'] = tvals_m[:, i]
            if localR2_m is not None:
                map_df['Local_R2'] = localR2_m.flatten() if hasattr(localR2_m, 'flatten') else localR2_m
